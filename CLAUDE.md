...
- **Sprint 6b:** Session entity como objeto de primera clase + Post-sesión import explícito (cierra el loop Studio → General stats vía la tercera capa de identidad). Implementación: modelo `StudioSession` en DB + LangGraph 1.x checkpointer (Postgres) sobre el writing graph. Objetivos centrales: (a) la capa Post-sesión del modelo de identidad gana infraestructura real; (b) Sprint 7 (LangMem episódico) y Sprint 6c (evals) ganan los episodios sobre los que operar; (c) resumibilidad como side effect.
- **Sprint 6b.5:** Writer initialization flow ("quiero un escritor tipo GRRM"). Wizard guiado conversacional que reemplaza el form rígido del CreateWriterModal. Free-text → identity inicial completa via Claude con structured output. Reusa el Store del checkpointer de 6b para snapshots intermedios. Empezar simple — refactor a deepagents solo si se queda corto. Separado de 6b para no inflar el sprint.
- **Sprint 6c:** LangSmith setup + evals del evolution pipeline. Datasets desde traces reales, LLM-as-judge para `should_evolve` y coherencia de cambios. Antes de buscar usuarios reales — para detectar regresiones cuando el evolution pipeline cambie.
  - **Pendiente a revisar dentro de 6c:** verificado en prod 2026-04-08 — el detector dispara correctamente con mensajes explícitos de identity-shaping >15 palabras (probado con "Quiero que de ahora en adelante seas...", evolucionó identity v1→v3 con cambios coherentes en personality/emotions/topics/lifelong_objectives). El gap real es que el umbral conservador del prompt + rate limit de palabras hacen que mensajes naturales/cortos casi nunca triggereen — un usuario casual probablemente nunca lo gatille sin coaching. Calibrar `EVOLUTION_DETECT_PROMPT` y/o el umbral con datasets reales en LangSmith.
